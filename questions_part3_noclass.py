...
def _svc_metrics(clf: SVC, x: NDArray[np.floating], y: NDArray[np.int32]) -> tuple[StratifiedKFold, dict[str, Any], dict[str, float], dict[str, float]]:
...
def part_3c(
    ...
    if not set(np.unique(y_train)).issubset({0, 1}):
        part_3b(x_train, y_train, x_test, y_test)

    clf = SVC(random_state=seed)
    cv, _, mean_metrics, std_metrics = _svc_metrics(clf, x_train, y_train)
    clf.fit(x_train, y_train)
    conf_mat = confusion_matrix(y_test, clf.predict(x_test), labels=[0, 1])
