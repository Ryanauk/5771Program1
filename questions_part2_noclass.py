...
def part_2b(
    ...
    cv_1d = ShuffleSplit(n_splits=5, random_state=seed)
    scores_1d = _summary(cross_validate(clf_1d, xtr, ytr, cv=cv_1d, scoring="accuracy"))

    clf_1f = LogisticRegression(max_iter=500, random_state=seed)
    cv_1f = ShuffleSplit(n_splits=5, random_state=seed)
    ...
    # Grid-search Logistic Regression for the requested 1g-like outputs on multiclass data.
    clf_1g = LogisticRegression(max_iter=500, random_state=seed, solver="saga", tol=0.001)
    clf_1g.fit(xtr, ytr)
    grid_search = GridSearchCV(
        LogisticRegression(max_iter=500, random_state=seed, solver="saga", tol=0.001),
        param_grid={"fit_intercept": [True], "C": [0.5], "penalty": ["l1"]},
        cv=3,
        refit=True,
    )
    ...
    for n in ntrain_list:
        ...
        clf_n = LogisticRegression(max_iter=500, random_state=seed)
    ...
    answers["clf_1g"] = clf_1g
    answers["default_parameters_1g"] = {"fit_intercept": True, "C": 1.0, "penalty": ["l2"]}
